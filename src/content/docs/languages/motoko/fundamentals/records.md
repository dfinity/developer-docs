...
Unlike [tuples](/languages/motoko/fundamentals/tuples), which use positional access, records provide field-based access, improving readability and maintainability.
...
`person` is a record with two labeled fields, `name` of type [`Text`](https://mops.one/core/docs/Text) and `age` of type [`Nat`](https://mops.one/core/docs/Nat).
...
Records can be destructured using [`switch`](/languages/motoko/fundamentals/switch), allowing selective extraction of fields. This approach makes accessing deeply nested fields more explicit and readable.
