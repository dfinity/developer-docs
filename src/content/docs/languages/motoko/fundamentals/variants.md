...
Variant type describe values that take on one of several forms, each labeled with a distinct  tag. Unlike [records](/languages/motoko/fundamentals/records), where all fields exist at once, a value of a variant type holds exactly one of the type's possible values. This makes variants useful for representing mutually exclusive alternatives such as states, enumerations, categories and even trees.
...
`#Active` and `#Inactive` are constant tags, with an implicit `()` argument, meaning they only store trivial data. `#Banned` carries a [`Text`](https://mops.one/core/docs/Text) value, such as the reason for the ban.
...
To work with a variant, use a [`switch`](/languages/motoko/fundamentals/switch) expression to match each possible case.
...
Since the traffic light can only be in one of these states at a time, a variant is well-suited to model it. There is no invalid [state](/languages/motoko/fundamentals/state), as every possible value is explicitly defined. The transitions are controlled and predictable.
...
A function can define how the traffic light cycles from one [state](/languages/motoko/fundamentals/state) to the next.
...
1. `#node` contains a value of type [`Nat`](https://mops.one/core/docs/Nat) and two child trees (`left` and `right`).
...
Currently, the example tree only supports [`Nat`](https://mops.one/core/docs/Nat) values. To allow it to store any type of data, a [generic type](/languages/motoko/fundamentals/advanced-types#generic-types) can be used. A generic type allows a data structure to work with multiple types by using a placeholder type `T`, which is replaced with a specific type when used.
...
With this change, the tree can store any type, such as [`Text`](https://mops.one/core/docs/Text), [`Nat`](https://mops.one/core/docs/Nat), or custom types, making it more flexible and reusable.
