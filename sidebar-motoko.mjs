 ...
 *
 * TRANSITIONAL: this file exists only until the upstream motoko repo adds
 * metadata index.md files to each fundamentals/ subdirectory (with
 * sidebar.order) and sidebar: { hidden: true } to base-core-migration.md.
 * Once those upstream changes land, delete this file and replace it with:
 *
 *   motokoSidebar = {
 *     label: "Motoko",
 *     collapsed: true,
 ...
 *   }
 *
 * Until then, the fundamentals/ section must be listed explicitly because
 * subdirectory names sort alphabetically after numeric prefix removal and
 * autogenerate cannot recover the correct display order without index.md files.
 *
 * Maintenance: if the upstream adds a new fundamentals page before the
 * metadata index.md changes land, add it to the explicit list below.
 * icp-features/ and reference/ are already autogenerated — new pages there
