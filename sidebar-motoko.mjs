 ...
 *
 * Fundamentals uses explicit items rather than autogenerate because Starlight
 * v0.38 uses raw directory keys as group labels (so `actors/` → "actors" instead
 * of "Actors"). Once Starlight promotes index.md frontmatter labels to group
 * headers, the explicit items can be replaced with autogenerate per section.
 *
 * icp-features/ uses autogenerate — it is a flat list of pages with correct
 * sidebar.order frontmatter and no subdirectories.
 *
 * reference/ uses explicit items to preserve the "Motoko changelog" label
 * (upstream changelog.md title is just "Changelog").
 */
  ...
  items: [
    ...
    },
    {
      ...
      items: [
        ...
        { slug: "languages/motoko/reference/changelog", label: "Motoko changelog" },
      ],
    },
  ],
};
