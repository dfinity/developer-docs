...
- Add a new page under `guides/frontends/` to `sidebar.mjs` as well: that one group lists its pages explicitly, so `sidebar.order` alone does not place it (the group's own pages under `static-site/` are autogenerated and need nothing)
