...
// Rewrite every public-docs link found via PATH_MAP. Unknown links are left
// untouched so the validation pass can flag them.
function rewriteDocsLinks(text) {
  return text.replace(DOCS_LINK, (url) => PATH_MAP[canonicalize(url)] ?? url);
}

// Trailing marker appended to each generated spec. Flags the file as synced
// (do not hand-edit — the next sync overwrites it) and records the upstream
// source, per the repo's `<!-- Upstream: -->` convention.
function generatedMarker(upstream) {
  return (
    '\n<!-- Generated by scripts/sync-ii-spec.mjs — do not edit directly; ' +
...
// 3. Rewrite links
content = rewriteDocsLinks(content);
...
// 7. Append the generated-file marker and Upstream comment
content =
  content.trimEnd() +
  '\n' +
  generatedMarker('sync from dfinity/internet-identity — docs/ii-spec.mdx, src/internet_identity/internet_identity.did');
...
vcContent = rewriteDocsLinks(vcContent);
...
// 4. Append the generated-file marker and Upstream comment
vcContent =
  vcContent.trimEnd() +
  '\n' +
  generatedMarker('sync from dfinity/internet-identity — docs/vc-spec.md');
