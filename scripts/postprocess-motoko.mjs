...
function transformEntry(entry) {
  ...
  if ('autogenerate' in entry) {
    ...
    // Starlight >= 0.39 removed labelled autogenerated groups: the autogenerate
    // config must sit inside an `items` array. Upstream may still use the older
    // `{ label, autogenerate }` shape, so normalise it here rather than depending
    // on caffeinelabs/motoko to migrate. Already-nested entries (no `label`) pass
    // through untouched. `collapsed` is copied onto the autogenerate config so
    // nested subgroups keep the parent's collapsed state, which they no longer
    // inherit as of 0.39.
    if ('label' in entry) {
      const { autogenerate: _dropped, ...group } = entry;
      if (group.collapsed) autogenerate.collapsed = true;
      return { ...group, items: [{ autogenerate }] };
    }
    return { ...entry, autogenerate };
  }
  if ('items' in entry) {
