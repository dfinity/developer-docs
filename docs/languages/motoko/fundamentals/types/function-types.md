...
- **Shared functions**, declared using the `shared`, `shared query`, or `shared composite query` keywords, are asynchronous by nature. Calling a shared function sends a message to another actor. The caller is typically another Motoko actor, a [canister](/concepts/canisters), or an [agent](/guides/canister-calls/calling-from-clients).
...
[Composite queries](/concepts/canisters#composite-queries) chain multiple query calls together within the same function.
