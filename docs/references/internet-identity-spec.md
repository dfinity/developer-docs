...
That origin is normalized onto `ic0.app` when it is a canister gateway origin, since principals are derived against that domain (see [Identity design and data model](#identity-design-and-data-model)), but a canister is not necessarily served on every gateway domain: one reachable at `https://<canister id>.icp0.io` can answer `400 client_domain_canister_mismatch` at `https://<canister id>.ic0.app`. So for an origin of the form `https://<canister id>.<gateway domain>`, Internet Identity asks that origin first and then the same canister on the remaining gateway domains, stopping at the first one that answers for the application: a `200` (whatever the document it carries turns out to be) or a `404` settles the question, while a `400`, a `5xx` or no response at all moves on to the next domain. All three resolve to the same canister, so this only widens where the document may be served, never whose document is used, and an application that publishes no document is still asked only once. An origin that is not a canister gateway origin (a custom domain, for example) is asked as given, and only it.
...
The complete Candid interface definition is available at [`internet-identity.did`](/references/internet-identity.did). This file defines all types and method signatures in machine-readable Candid format and can be used for binding generation and type checking.
...
<!-- Generated by scripts/sync-ii-spec.mjs — do not edit directly; changes are overwritten on the next sync. -->
