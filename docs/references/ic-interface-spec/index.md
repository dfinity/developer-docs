...
-   Given a payload together with public key and signature in the format described above the signature can be verified by checking the following two conditions:
    ...
    -   If the `certificate` includes a subnet delegation, then the `signing_canister_id` must be included in the delegation's canister id range (see [Delegation](./certification.md#certification-delegation)), and the subnet type (obtained by looking up the path `/subnet/<subnet_id>/type` in the delegation's certificate) *must* be present and it *must not* be `cloud_engine`.
