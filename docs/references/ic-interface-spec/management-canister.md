...
Any unspent part of the per-node budgets is credited to the caller's cycles balance asynchronously, separately from the refund that accompanies the response: each node returns what it did not spend. A node that never reports has its whole budget returned after a timeout of one minute. Therefore, a canisters cycle balance may continue to settle for a while after a response was received, while additional asynchronous refunds are received.
