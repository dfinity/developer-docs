...
{/* Needs human verification: wasm_memory_limit default may now be 3 GiB (3,221,225,472 bytes) per CDK docs rather than 0 */}
...
{/* Needs human verification: icp canister settings update flag for snapshot_visibility may not be implemented yet — use icp.yaml configuration for now */}

| Property | Value |
|----------|-------|
| Type | `controllers` or `public` |
| Default | `controllers` |
...
```yaml
settings:
  snapshot_visibility: controllers  # or "public"
```
...
persistent actor Self {
  ...
  type CanisterSettings = {
    ...
    environment_variables : ?[(Text, Text)];
  };
