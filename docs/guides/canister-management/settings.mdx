...
Like compute allocation, memory allocation incurs a rental fee based on time and allocated amount, regardless of actual usage. See [cycles costs](../../reference/cycles-costs.md) for pricing.

### Freezing threshold

The minimum time (in seconds) the canister should be able to survive on its current cycle balance. If the balance drops below what is needed to sustain this duration, the canister freezes.
...
For mainnet canisters, add `-e production` (or whatever your mainnet environment is named):

```bash
icp canister settings update backend -e production --freezing-threshold 90d
```

For the full list of CLI flags, see the [icp-cli reference](https://cli.internetcomputer.org/).
...
For the full management canister interface, see the [management canister reference](../../reference/management-canister.md).
...
{/* Upstream: informed by dfinity/portal — docs/building-apps/canister-management/settings.mdx, docs/building-apps/canister-management/control.mdx; dfinity/icp-cli — docs/reference/canister-settings.md, docs/reference/cli.md; dfinity/icskills — skills/cycles-management/SKILL.md */}
