...
:::caution
Reading a snapshot exposes the canister's full state: the Wasm module, Wasm memory, stable memory, and chunk store. Granting snapshot access is closer to handing out a state dump than to granting a read-only report.
:::
...
Each of the three visibility settings takes either a fixed policy (`--log-visibility`, `--snapshot-visibility`, `--status-visibility`, each accepting `controllers` or `public`) or a list of principals (`--add-<kind>-viewer`, `--set-<kind>-viewer`, `--remove-<kind>-viewer`). The two forms conflict: a viewer list has no principals to add to or remove from while the policy is `public`, so replace a public policy with `--set-<kind>-viewer` and revoke it with `--<kind>-visibility controllers`.
