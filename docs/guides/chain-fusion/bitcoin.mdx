...
async fn withdraw(btc_address: String, amount: u64) -> RetrieveBtcResult {
    ...
    let approve_args = ApproveArgs {
        ...
        amount: Nat::from(amount) + Nat::from(10u64),
...
The Rust CDK's `ic_cdk::bitcoin_canister` module also provides `get_blockchain_info()` for querying blockchain state (current height, chain tip hash, etc.):

```rust
// Query blockchain state (height, chain tip, etc.)
// let info = ic_cdk::bitcoin_canister::get_blockchain_info().await;
```
...
All Bitcoin API calls require cycles. The Rust CDK's `ic_cdk::bitcoin_canister` module handles cycle calculation and attachment automatically.
...
All Bitcoin API calls require cycles attached to the call. The Rust CDK handles this automatically through the `ic_cdk::bitcoin_canister` module. In Motoko, attach cycles explicitly with `(with cycles = amount)`.
...
{/* Upstream: informed by dfinity/portal — docs/build-on-btc/*; dfinity/icskills — skills/ckbtc/SKILL.md; dfinity/icp-cli-templates — bitcoin-starter/ */}
