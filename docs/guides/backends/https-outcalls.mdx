...
| Methods | `GET`, `HEAD`, `POST` | plus `PUT`, `DELETE`, `PATCH` | plus `PUT`, `DELETE`, `PATCH` when `total_requests`, `min_responses`, and `max_responses` are equal |
...
| Role of `max_response_bytes` | sets both the limit and the price | sets the limit and, through the worst-case usage computed from it, the size of the reservation; not the price |
...
>
> Flexible outcalls do run locally: the local replica reports 13 nodes through `ic0.subnet_self_node_count`, so a committee of that size is accepted and the call returns as many responses as you ask for. They all come from the one node that is actually running, so a local run exercises the call and your reconciliation code, but never the disagreement between nodes that reconciliation exists for.
...
{/* Upstream: informed by dfinity/portal docs/building-apps/network-features/using-http/https-outcalls/; dfinity/examples send_http_get, send_http_post, send_http_flexible */}
