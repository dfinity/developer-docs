...
2. **Serve it with the right content type and CORS headers.** II reads the file cross-origin, and nothing is set for you.
   ...
   On the [static-site recipe](../frontends/static-site/overview.md), `.well-known/` is uploaded automatically; declare the two headers in a `_headers` file at the root of your build directory:
