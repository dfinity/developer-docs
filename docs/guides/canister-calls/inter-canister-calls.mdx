...
A canister that charges per-call defines a required fee, rejects calls that don't meet it, and accepts exactly that amount. Any cycles above the fee are returned to the caller automatically:
...
persistent actor {
  ...
  public func deposit() : async Nat {
    ...
    Cycles.accept<system>(fee) // accept exactly the fee; excess is returned automatically
  };
}
```
...
fn deposit() -> Nat {
    ...
    Nat::from(ic_cdk::api::msg_cycles_accept(FEE)) // accept exactly the fee; excess is returned automatically
}
```
