...
You can combine cycle attachment with a bounded-wait timeout in the same parenthetical:

```motoko
let result = await (with timeout = 25; cycles = 500_000_000) target.someMethod();
```
...
`.with_cycles()` and `.change_timeout()` can be chained in any order on the same `Call`.
...
### Accepting cycles in the callee

A canister that charges per-call claims the attached cycles with `Cycles.accept` (Motoko) or `msg_cycles_accept` (Rust). Any cycles not accepted are returned to the caller automatically:
