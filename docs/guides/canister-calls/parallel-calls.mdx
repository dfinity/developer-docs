...
In Rust, [`Call::bounded_wait`](https://docs.rs/ic-cdk/latest/ic_cdk/call/struct.Call.html) returns an `IntoFuture`. You can collect these futures into a `Vec` and pass them to [`futures::future::join_all`](https://docs.rs/futures/latest/futures/future/fn.join_all.html), which awaits all of them together.
...
pub async fn parallel_calls(n: u64) -> u64 {
    ...
    // Build all futures before awaiting any of them — each into_future()
    // sends the request message immediately.
...
The IC enforces a limit on the number of in-flight calls a canister can have outstanding to any other single canister. Dispatching more calls than this limit causes the excess to be rejected immediately. Sequential calls stay within the limit because only one call is in-flight at a time. Parallel calls can exceed it when `n` is large.
...
thread_local! {
    static PARTITIONS: Arc<RwLock<Vec<Principal>>> = Arc::new(RwLock::new(vec![]));
}
