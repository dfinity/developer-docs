...
- **Motoko**: `Nat.toNat64(Int.abs(Time.now()))` (as shown in `sendTokens` above)
...
> To grant or inspect an allowance manually without writing code, use the [`icp token approve`](https://cli.internetcomputer.org/1.4/reference/cli#icp-token-approve) and [`icp token allowance`](https://cli.internetcomputer.org/1.4/reference/cli#icp-token-allowance) CLI commands.
...
func depositAccount(canister : Principal, user : Principal) : Account {
  ...
  let subaccount = Array.tabulate<Nat8>(32, func(i) {
    if (i == 0) { Nat.toNat8(bytes.size()) }
    else if (i <= bytes.size()) { bytes[i - 1] }
    else { 0 }
  });
  { owner = canister; subaccount = ?Array.toBlob(subaccount) }
};
```
