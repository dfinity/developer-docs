...
* **Bound outcall budgets**: under HTTPS outcall pricing version 2, the cycles you attach beyond the base fee are the call's spending limit, and the remote server decides how much of it is consumed. Attach what the call is expected to need rather than a generous margin, especially where the outcall can be triggered by a user, and the remote server is untrusted. See [HTTPS outcalls security](https-outcalls.md#bound-the-cycles-you-attach-under-pricing-version-2).
