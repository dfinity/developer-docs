...
For end-to-end tests that need a full network with all system canisters, use a containerized network instead of PocketIC. See the [icp-cli containerized networks documentation](https://cli.internetcomputer.org/1.3/guides/containerized-networks) for how to configure Docker-based test networks in `icp.yaml`.
