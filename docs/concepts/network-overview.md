...
2. **API endpoint.** Agent libraries (like `@dfinity/agent` in JavaScript) send ingress messages to boundary nodes, which forward them to the target canister's subnet.
