...
3. Update `sidebar.mjs`: switch `reference/` to `autogenerate` (all six files
   will have sequential `sidebar.order` 1–6). `icp-features/` is already
   autogenerated. Keep `fundamentals/` as an explicit items list — subdirectory
   order is not recoverable from file names or frontmatter after prefix removal.
